...
def create_tables(cur):
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS clients_manage(
    ...
    client_id INTEGER NOT NULL REFERENCES clients_manage(id),
...
def add_new_client(cur, client_name, client_surname, client_email):
    '''Добавление нового клиента в таблицу clients_manage'''
    cur.execute("""
    INSERT INTO clients_manage(client_name, client_surname, client_email) VALUES(%s, %s, %s);
...
def change_client_data():
    ...
    while True:
        ...
        if command_symbol == 1:
            ...
            cur.execute("""
            UPDATE clients_manage SET client_name=%s WHERE id=%s;
            ...
            cur.execute("""
            UPDATE clients_manage SET client_surname=%s WHERE id=%s;
            ...
            cur.execute("""
            UPDATE clients_manage SET client_email=%s WHERE id=%s;
...
def delete_client():
    ...
    with conn.cursor() as cur:
        ...
        #удаление информации о клиенте из таблицы clients_manage
        cur.execute("""
        DELETE FROM clients_manage WHERE id=%s AND client_surname=%s
...
def find_client():
    ...
    while True:
        ...
        if input_command_for_finding == 1:
            ...
            FROM clients_manage AS cm
            LEFT JOIN client_phonenumbers AS cp ON cp.id_phonenumber = cm.id
            WHERE client_name=%s
...
with psycopg2.connect(database="netology_db", user="postgres", password="Astrah-20047") as conn:
    with conn.cursor() as cur:
        create_tables(cur)
        add_new_client(cur, "Lionel", "Messi", "messi@g.com")
        add_new_client(cur, "Cristiano", "Ronaldo", "cri_ro@g.com")
        add_new_client(cur, "Eden", "Azar", "azar@g.com")
        add_new_client(cur, "David", "Alaba", "alaba@g.com")
        add_new_client(cur, "Romeru", "Lukaku", "lukaku@g.com")
        add_new_phonenumber(cur, 1, "1010101010")
        add_new_phonenumber(cur, 2, "7777777777")
        add_new_phonenumber(cur, 3, "1414141414")
        add_new_phonenumber(cur, 4, "3333333333")
        add_new_phonenumber(cur, 5, "1111111111")
        change_client_data()
